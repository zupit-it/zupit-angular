...
export class AuthExpiredInterceptor implements HttpInterceptor {
  ...
  constructor(
    ...
    @Inject(SESSION_EXPIRED_REDIRECT_URL)
    private sessionExpiredRedirectUrl: string | undefined,
    @Inject(REFRESH_TOKEN) private refreshToken: boolean = false,
    @Inject(UNAUTHORIZED_URL_BLACKLIST)
    private unauthorizedUrlBlacklist: string[]
  ) {}
  ...
  intercept(
    ...
    return next.handle(request).pipe(
      catchError((err: HttpErrorResponse) => {
        if (err.status === 401) {
          if (
            this.authenticationService.hasRefreshToken() &&
            this.refreshToken &&
