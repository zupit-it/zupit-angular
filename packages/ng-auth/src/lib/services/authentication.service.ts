...
export class AuthenticationService {
  ...
  public initialize(): Observable<UserType> {
    return this.doInitialize().pipe(
      tap((user) =>
        this.events$.next(new AuthenticationEvent('initialized', user))
      )
    )
  }
  ...
  public refreshToken(): Observable<string> {
    ...
    return this.authenticationProvider
      .refreshToken(accessToken, refreshToken, metadata)
      .pipe(
        tap((newAccessToken) => {
          ...
          this.store(
            this.AUTH_METADATA,
            JSON.stringify(newAccessToken.metadata)
          )
        }),
  ...
  notifyGuardBlockedAccess(
    guardName: string,
    route?: ActivatedRouteSnapshot | Route,
    state?: RouterStateSnapshot
  ): void {
    this.events$.next(
      new AuthenticationEvent('guard-blocked-access', this.authenticationUser, {
        guardName,
        route,
        state
      })
